# 技术架构设计文档

> **作者**: 孙顺达
> **项目**: 基于 Midscene.js 的 AI 驱动自动化测试平台
> **版本**: v1.0
> **日期**: 2025-11-12

---

## 📋 目录

- [1. 架构概览](#1-架构概览)
- [2. 技术选型](#2-技术选型)
- [3. 核心模块设计](#3-核心模块设计)
- [4. 接口设计](#4-接口设计)
- [5. 数据流设计](#5-数据流设计)
- [6. Python-Node.js 集成方案](#6-python-nodejs-集成方案)
- [7. 安全性设计](#7-安全性设计)
- [8. 性能优化方案](#8-性能优化方案)
- [9. 部署方案](#9-部署方案)
- [10. 监控和日志](#10-监控和日志)

---

## 1. 架构概览

### 1.1 整体架构

```
┌──────────────────────────────────────────────────────────────┐
│                        用户界面层                             │
│                      Gradio WebUI                             │
│  ┌────────┬────────┬────────┬────────┬────────┬────────┐    │
│  │ LLM配置│浏览器  │测试用例│PRD生成 │测试执行│操作说明│    │
│  │        │设置    │管理    │用例    │        │        │    │
│  └────────┴────────┴────────┴────────┴────────┴────────┘    │
└──────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────┐
│                      业务逻辑层 (Python)                      │
│  ┌────────────────┬────────────────┬────────────────┐       │
│  │PRDProcessor    │TestCaseManager │ReportGenerator │       │
│  │- parse_prd()   │- parse_yaml()  │- parse_report()│       │
│  │- generate_yaml│- validate_yaml()│- generate_html()│       │
│  │- call_llm()    │- execute_yaml() │- export_pdf()  │       │
│  └────────────────┴────────────────┴────────────────┘       │
└──────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────┐
│                   Midscene 桥接层 (Node.js)                   │
│  ┌──────────────────────────────────────────────────┐       │
│  │ execute.js                                       │       │
│  │ - executeYaml(yamlPath, config)                  │       │
│  │ - createBrowser()                                │       │
│  │ - createAgent()                                  │       │
│  │ - handleError()                                  │       │
│  └──────────────────────────────────────────────────┘       │
│                    通信方式: subprocess                       │
│                    数据格式: JSON (stdin/stdout)             │
└──────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────┐
│                   Midscene 核心层 (TypeScript)                │
│  ┌────────┬────────┬────────┬────────┬────────────┐        │
│  │ Agent  │ YAML   │ Insight│TaskCache│AIModel    │        │
│  │        │ Player │        │        │Integration │        │
│  └────────┴────────┴────────┴────────┴────────────┘        │
└──────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────┐
│               浏览器自动化层 (Playwright/Puppeteer)           │
└──────────────────────────────────────────────────────────────┘
                              ↓
                        ┌───────────┐
                        │  浏览器    │
                        └───────────┘
```

### 1.2 技术栈总览

| 层级 | 技术 | 版本 | 用途 |
|------|------|------|------|
| **前端** | Gradio | 4.16.0 | Web UI 框架 |
| **后端** | Python | 3.9+ | 业务逻辑处理 |
| **自动化引擎** | Midscene.js | 0.30.8 | 测试用例执行 |
| **浏览器驱动** | Playwright | 1.40+ | 浏览器控制 |
| **LLM** | DeepSeek-V3 | - | PRD 分析、用例生成 |
| **VLM** | Qwen2.5-VL | - | 视觉理解、元素定位 |
| **数据格式** | YAML | - | 测试用例标准格式 |
| **配置管理** | .env | - | 环境变量管理 |

---

## 2. 技术选型

### 2.1 前端框架: Gradio

**选择理由**:
- ✅ Python 原生,无需学习 JavaScript
- ✅ 快速构建,开发效率高
- ✅ 内置组件丰富 (文件上传、代码编辑器等)
- ✅ 适合内部工具和 Demo

**替代方案**:
- Streamlit: 类似 Gradio,但组件相对简单
- Flask + Vue.js: 灵活但开发周期长

### 2.2 后端语言: Python

**选择理由**:
- ✅ Gradio 是 Python 框架
- ✅ AI/ML 生态丰富 (LLM SDK、数据处理等)
- ✅ 开发效率高
- ✅ 易于维护

### 2.3 自动化引擎: Midscene.js

**选择理由**:
- ✅ 成熟稳定 (0.30.8 版本)
- ✅ 完整的 YAML 支持
- ✅ 内置报告生成
- ✅ 内置缓存机制
- ✅ 多平台支持 (Web/Android/iOS)

**为什么不用 browser-use?**
- Midscene YAML 格式更标准
- Midscene 功能更完整 (报告、缓存)
- Midscene 社区更活跃

### 2.4 浏览器驱动: Playwright

**选择理由**:
- ✅ 现代化 API
- ✅ 跨浏览器支持
- ✅ Midscene 已集成
- ✅ 文档完善

**备选方案**: Puppeteer (兼容性更好)

### 2.5 LLM 服务商

| 用途 | 模型 | 提供商 | 理由 |
|------|------|--------|------|
| PRD 分析 | DeepSeek-V3 | DeepSeek | 推理能力强、成本低 |
| 用例生成 | DeepSeek-V3 | DeepSeek | 生成质量高 |
| 元素定位 | Qwen2.5-VL | SiliconFlow | 视觉理解能力强 |
| 任务规划 | Qwen2.5-VL | SiliconFlow | 性价比高 |

---

## 3. 核心模块设计

### 3.1 PRDProcessor (PRD 处理器)

**职责**: 处理 PRD 文档,生成 YAML 测试用例

**类图**:
```
PRDProcessor
├── parse_prd(prd_content: str) -> Dict
│   └── 解析 PRD,提取关键信息
├── generate_yaml(prd_content: str, context?: str) -> Dict
│   └── 调用 LLM 生成 YAML
├── _call_llm_api(prompt: str) -> str
│   └── 调用 DeepSeek API
└── _extract_yaml_from_response(response: str) -> str
    └── 从 LLM 响应中提取 YAML
```

**接口定义**:

```python
class PRDProcessor:
    def __init__(self):
        self.api_key: str
        self.base_url: str
        self.model: str
        self.prompt_template: str

    def parse_prd(self, prd_content: str) -> Dict:
        """
        解析 PRD 文档

        Args:
            prd_content: PRD 文本内容

        Returns:
            {
                'content': str,      # 完整内容
                'length': int,       # 字符数
                'word_count': int,   # 单词数
                'has_url': bool      # 是否包含 URL
            }
        """

    def generate_yaml(
        self,
        prd_content: str,
        additional_context: Optional[str] = None
    ) -> Dict:
        """
        生成 YAML 测试用例

        Args:
            prd_content: PRD 内容
            additional_context: 额外上下文 (可选)

        Returns:
            成功:
            {
                'status': 'success',
                'yaml_content': str,
                'message': str
            }

            失败:
            {
                'status': 'error',
                'message': str
            }
        """
```

**数据流**:
```
PRD 文本
    ↓
parse_prd() → 提取信息
    ↓
generate_yaml() → 构建 Prompt
    ↓
_call_llm_api() → 调用 DeepSeek API
    ↓
_extract_yaml_from_response() → 提取 YAML
    ↓
返回 YAML 文本
```

---

### 3.2 TestCaseManager (测试用例管理器)

**职责**: 解析、验证和执行 YAML 测试用例

**类图**:
```
TestCaseManager
├── parse_yaml(yaml_content: str) -> Dict
│   └── 解析 YAML 格式
├── validate_yaml(parsed_data: Dict) -> Dict
│   └── 验证 YAML 格式和必填字段
├── extract_tasks(parsed_data: Dict) -> List[Dict]
│   └── 提取任务列表
├── execute_yaml(yaml_content: str, config?: Dict) -> Dict
│   └── 执行 YAML 测试用例
└── _call_node_script(yaml_path: str, config: Dict) -> Dict
    └── 调用 Node.js 执行脚本
```

**接口定义**:

```python
class TestCaseManager:
    def __init__(self):
        self.node_script_path: str  # Node.js 脚本路径

    def parse_yaml(self, yaml_content: str) -> Dict:
        """
        解析 YAML 测试用例

        Returns:
            成功:
            {
                'status': 'success',
                'data': dict,           # 解析后的 Python 字典
                'message': str
            }

            失败:
            {
                'status': 'error',
                'message': str          # 错误信息
            }
        """

    def validate_yaml(self, parsed_data: Dict) -> Dict:
        """
        验证 YAML 格式

        Returns:
            {
                'valid': bool,
                'errors': List[str],    # 错误列表
                'warnings': List[str]   # 警告列表
            }
        """

    def extract_tasks(self, parsed_data: Dict) -> List[Dict]:
        """
        提取任务列表

        Returns:
            [
                {
                    'index': int,
                    'name': str,
                    'steps_count': int,
                    'description': str
                },
                ...
            ]
        """

    def execute_yaml(
        self,
        yaml_content: str,
        config: Optional[Dict] = None
    ) -> Dict:
        """
        执行 YAML 测试用例

        Args:
            yaml_content: YAML 内容
            config: 执行配置
                {
                    'headless': bool,
                    'outputDir': str,
                    'timeout': int
                }

        Returns:
            成功:
            {
                'status': 'success',
                'data': {
                    'report_path': str,
                    'duration': float,
                    'tasks_count': int,
                    'timestamp': str
                },
                'message': str
            }

            失败:
            {
                'status': 'error',
                'message': str,
                'details': dict
            }
        """
```

**执行流程**:
```
YAML 内容
    ↓
保存到临时文件 /tmp/testcase_xxx.yaml
    ↓
构建 subprocess 命令
    ↓
调用 Node.js: node execute.js <yaml_path> <config_json>
    ↓
Node.js 执行 Midscene
    ↓
返回结果 (JSON via stdout)
    ↓
解析结果并返回
    ↓
清理临时文件
```

---

### 3.3 ReportGenerator (报告生成器)

**职责**: 解析和展示测试报告

**类图**:
```
ReportGenerator
├── parse_midscene_report(report_path: str) -> Dict
│   └── 解析 Midscene HTML 报告
├── extract_summary(report_data: Dict) -> Dict
│   └── 提取报告摘要
└── generate_html(report_data: Dict) -> str
    └── 生成自定义 HTML 报告 (迭代 2)
```

**接口定义**:

```python
class ReportGenerator:
    def parse_midscene_report(self, report_path: str) -> Dict:
        """
        解析 Midscene 生成的报告

        Returns:
            {
                'status': 'success',
                'report_path': str,
                'url': str  # file:// URL
            }
        """

    def extract_summary(self, report_data: Dict) -> Dict:
        """
        提取报告摘要

        Returns:
            {
                'total_tasks': int,
                'passed_tasks': int,
                'failed_tasks': int,
                'duration': float,
                'status': str  # 'success' | 'failure'
            }
        """
```

---

## 4. 接口设计

### 4.1 Python → Node.js 接口

**通信方式**: subprocess

**命令格式**:
```bash
node execute.js <yaml_path> <config_json>
```

**配置参数 (JSON)**:
```json
{
  "headless": false,
  "outputDir": "./reports",
  "groupName": "测试执行",
  "cache": false,
  "viewport": {
    "width": 1920,
    "height": 1080
  },
  "timeout": 300000
}
```

**返回格式 (stdout)**:

成功:
```json
{
  "status": "success",
  "result": {
    "username": "test"
  },
  "report_path": "/path/to/report.html",
  "duration": 45.3,
  "tasks_count": 2,
  "timestamp": "2025-01-12T10:30:00.000Z"
}
```

失败:
```json
{
  "status": "error",
  "message": "Error message",
  "stack": "Error stack trace",
  "timestamp": "2025-01-12T10:30:00.000Z"
}
```

### 4.2 LLM API 接口

**DeepSeek API**:

请求:
```http
POST https://api.deepseek.com/v1/chat/completions
Authorization: Bearer <API_KEY>
Content-Type: application/json

{
  "model": "deepseek-chat",
  "messages": [
    {
      "role": "system",
      "content": "你是一个专业的测试用例设计专家。"
    },
    {
      "role": "user",
      "content": "<prompt>"
    }
  ],
  "temperature": 0.7,
  "max_tokens": 4096
}
```

响应:
```json
{
  "id": "chatcmpl-xxx",
  "object": "chat.completion",
  "created": 1234567890,
  "model": "deepseek-chat",
  "choices": [
    {
      "index": 0,
      "message": {
        "role": "assistant",
        "content": "生成的内容"
      },
      "finish_reason": "stop"
    }
  ],
  "usage": {
    "prompt_tokens": 100,
    "completion_tokens": 200,
    "total_tokens": 300
  }
}
```

### 4.3 Gradio 事件接口

**按钮点击事件**:
```python
button.click(
    fn=handler_function,           # 处理函数
    inputs=[input1, input2],       # 输入组件列表
    outputs=[output1, output2]     # 输出组件列表
)
```

**文件上传事件**:
```python
file_upload.change(
    fn=load_file_handler,
    inputs=[file_upload],
    outputs=[text_output]
)
```

---

## 5. 数据流设计

### 5.1 PRD → YAML 数据流

```
┌─────────────────────┐
│ 1. 用户上传 PRD     │
│    (Markdown/Text)  │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 2. Gradio 接收      │
│    gr.File 或       │
│    gr.Textbox       │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 3. PRDProcessor     │
│    .parse_prd()     │
│    提取信息         │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 4. 构建 Prompt      │
│    使用模板         │
│    + PRD 内容       │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 5. 调用 LLM API     │
│    DeepSeek-V3      │
│    生成 YAML        │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 6. 提取 YAML        │
│    从响应中提取     │
│    ```yaml ... ```  │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 7. 返回给前端       │
│    gr.Code 显示     │
└─────────────────────┘
```

### 5.2 YAML → 执行结果数据流

```
┌─────────────────────┐
│ 1. YAML 内容        │
│    (用户输入)       │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 2. TestCaseManager  │
│    .parse_yaml()    │
│    解析为 Python dict│
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 3. validate_yaml()  │
│    检查格式         │
└─────────────────────┘
          ↓ (valid)
┌─────────────────────┐
│ 4. 保存到临时文件   │
│    /tmp/test_xxx.yaml│
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 5. 构建 subprocess  │
│    cmd: node execute.js│
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 6. Node.js 执行     │
│    execute.js       │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 7. Midscene Agent   │
│    .runYaml()       │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 8. Playwright       │
│    浏览器操作       │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 9. 生成报告         │
│    report.html      │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 10. 返回结果 (JSON) │
│     via stdout      │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 11. Python 解析     │
│     json.loads()    │
└─────────────────────┘
          ↓
┌─────────────────────┐
│ 12. 前端展示        │
│     gr.Markdown     │
└─────────────────────┘
```

---

## 6. Python-Node.js 集成方案

### 6.1 方案对比

| 方案 | 优点 | 缺点 | 选择 |
|------|------|------|------|
| **subprocess** | 简单、稳定、无依赖 | 性能略低、通信开销 | ✅ 采用 |
| HTTP API | 异步、高性能 | 需要启动服务、复杂 | ❌ |
| gRPC | 高性能、类型安全 | 学习成本高、过度设计 | ❌ |
| 共享内存 | 性能最高 | 实现复杂、跨平台问题 | ❌ |

### 6.2 subprocess 实现细节

**Python 端**:

```python
import subprocess
import json
import os

def call_node_script(yaml_path: str, config: dict) -> dict:
    """
    调用 Node.js 脚本

    Args:
        yaml_path: YAML 文件路径
        config: 配置字典

    Returns:
        执行结果字典

    Raises:
        subprocess.TimeoutExpired: 超时
        subprocess.CalledProcessError: 执行失败
        Exception: 其他错误
    """

    # 构建命令
    cmd = [
        'node',
        'src/midscene_bridge/execute.js',
        yaml_path,
        json.dumps(config, ensure_ascii=False)
    ]

    # 执行
    try:
        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
            check=True,
            timeout=config.get('timeout', 300),  # 默认 5 分钟
            env=os.environ.copy()  # 继承环境变量
        )

        # 解析输出
        output = json.loads(result.stdout)
        return output

    except subprocess.TimeoutExpired:
        raise Exception('执行超时')

    except subprocess.CalledProcessError as e:
        # 解析错误信息
        try:
            error = json.loads(e.stderr)
        except:
            error = {'message': e.stderr}
        raise Exception(f"执行失败: {error.get('message', '未知错误')}")

    except json.JSONDecodeError:
        raise Exception('返回结果格式错误')
```

**Node.js 端**:

```javascript
// src/midscene_bridge/execute.js
import { PlaywrightAgent } from '@midscene/web/playwright';
import { chromium } from 'playwright';

async function main() {
  try {
    // 1. 解析参数
    const args = process.argv.slice(2);
    const yamlPath = args[0];
    const config = JSON.parse(args[1] || '{}');

    // 2. 验证参数
    if (!yamlPath) {
      throw new Error('Missing yaml_path argument');
    }

    // 3. 执行
    const result = await executeYaml(yamlPath, config);

    // 4. 输出结果 (stdout)
    console.log(JSON.stringify(result));

  } catch (error) {
    // 5. 输出错误 (stderr)
    console.error(JSON.stringify({
      status: 'error',
      message: error.message,
      stack: error.stack
    }));
    process.exit(1);
  }
}

async function executeYaml(yamlPath, config) {
  // ... 执行逻辑 ...
}

main();
```

### 6.3 错误处理

**错误类型**:

1. **参数错误**: YAML 路径不存在
2. **执行超时**: 超过 timeout 时间
3. **浏览器错误**: Playwright 启动失败
4. **网络错误**: 目标网站无法访问
5. **LLM 错误**: AI 模型调用失败
6. **YAML 错误**: 格式不正确

**处理策略**:

```python
def execute_yaml_with_error_handling(yaml_content, config):
    """执行 YAML 并处理各种错误"""

    try:
        # 执行
        result = testcase_manager.execute_yaml(yaml_content, config)

        if result['status'] == 'success':
            return {
                'success': True,
                'data': result['data']
            }
        else:
            return {
                'success': False,
                'error': result['message'],
                'category': classify_error(result['message'])
            }

    except subprocess.TimeoutExpired:
        return {
            'success': False,
            'error': '执行超时,请检查网络或增加超时时间',
            'category': 'timeout'
        }

    except Exception as e:
        return {
            'success': False,
            'error': str(e),
            'category': 'unknown'
        }

def classify_error(message: str) -> str:
    """错误分类"""
    if 'timeout' in message.lower():
        return 'timeout'
    elif 'network' in message.lower():
        return 'network'
    elif 'yaml' in message.lower():
        return 'yaml'
    elif 'api' in message.lower():
        return 'api'
    else:
        return 'unknown'
```

---

## 7. 安全性设计

### 7.1 API Key 安全

**存储方式**:
- ✅ 存储在 `.env` 文件
- ✅ `.env` 加入 `.gitignore`
- ✅ 提供 `.env.example` 模板

**展示方式**:
- ✅ UI 中使用 `type="password"` 隐藏
- ✅ 日志中不打印 API Key

**验证机制**:
- ✅ 提供"测试连接"功能
- ✅ 启动时检查 API Key 是否配置

### 7.2 文件上传安全

**限制措施**:
- ✅ 限制文件类型 (`.yaml`, `.yml`, `.md`, `.txt`)
- ✅ 限制文件大小 (< 10MB)
- ✅ 验证文件内容格式

**示例**:
```python
def validate_upload_file(file):
    """验证上传文件"""

    # 1. 检查文件大小
    max_size = 10 * 1024 * 1024  # 10MB
    if os.path.getsize(file.name) > max_size:
        raise ValueError('文件大小超过限制 (10MB)')

    # 2. 检查文件类型
    allowed_extensions = ['.yaml', '.yml', '.md', '.txt']
    ext = os.path.splitext(file.name)[1].lower()
    if ext not in allowed_extensions:
        raise ValueError(f'不支持的文件类型: {ext}')

    # 3. 检查文件内容
    try:
        with open(file.name, 'r', encoding='utf-8') as f:
            content = f.read()
            # 基本验证
            if len(content) == 0:
                raise ValueError('文件内容为空')
    except UnicodeDecodeError:
        raise ValueError('文件编码错误,请使用 UTF-8')

    return True
```

### 7.3 命令注入防护

**风险**: subprocess 执行用户输入的内容

**防护措施**:
- ✅ 不直接执行用户输入的命令
- ✅ 固定执行脚本路径
- ✅ 参数使用 JSON 传递,不使用 shell 拼接

**安全示例**:
```python
# ✅ 安全: 固定脚本,JSON 参数
cmd = [
    'node',
    '/path/to/execute.js',
    yaml_path,
    json.dumps(config)
]
subprocess.run(cmd, shell=False)  # shell=False

# ❌ 不安全: shell=True + 字符串拼接
cmd = f"node /path/to/execute.js {yaml_path}"
subprocess.run(cmd, shell=True)  # 危险!
```

---

## 8. 性能优化方案

### 8.1 缓存机制

**Midscene 内置缓存**:
```javascript
// 启用缓存
const agent = await PlaywrightAgent.create(page, {
  cache: {
    enabled: true,
    id: 'test-cache',
    strategy: 'read-write'
  }
});
```

**Python 端缓存**:
```python
from functools import lru_cache

@lru_cache(maxsize=128)
def parse_yaml(yaml_content: str):
    """解析 YAML (带缓存)"""
    return yaml.safe_load(yaml_content)
```

### 8.2 并发优化 (迭代 2)

**批量测试并行执行**:
```python
import asyncio
import concurrent.futures

async def execute_batch(test_cases: List[str]):
    """并行执行多个测试用例"""

    with concurrent.futures.ProcessPoolExecutor(max_workers=3) as executor:
        futures = [
            executor.submit(execute_yaml, tc)
            for tc in test_cases
        ]

        results = [f.result() for f in futures]

    return results
```

### 8.3 LLM 调用优化

**流式响应** (迭代 2):
```python
def generate_yaml_stream(prd_content: str):
    """流式生成 YAML"""

    # 启用 stream
    response = requests.post(
        url,
        json={
            'model': 'deepseek-chat',
            'messages': messages,
            'stream': True
        },
        stream=True
    )

    for line in response.iter_lines():
        # 解析增量内容
        chunk = parse_sse(line)
        yield chunk
```

---

## 9. 部署方案

### 9.1 本地部署

**环境要求**:
- Python 3.9+
- Node.js 18.19+
- 8GB RAM
- 磁盘空间 10GB+

**部署步骤**:

```bash
# 1. 克隆项目
git clone https://github.com/xxx/midscene-auto-test.git
cd midscene-auto-test

# 2. 创建虚拟环境
python3 -m venv venv
source venv/bin/activate

# 3. 安装 Python 依赖
pip install -r requirements.txt

# 4. 安装 Node.js 依赖
npm install

# 5. 安装浏览器
npx playwright install chromium

# 6. 配置环境变量
cp .env.example .env
# 编辑 .env,填入 API Key

# 7. 启动应用
python src/ui/app.py
```

### 9.2 Docker 部署 (迭代 3)

**Dockerfile**:
```dockerfile
FROM python:3.9-slim

# 安装 Node.js
RUN curl -fsSL https://deb.nodesource.com/setup_18.x | bash -
RUN apt-get install -y nodejs

# 安装 Playwright 依赖
RUN apt-get install -y \
    libnss3 \
    libatk-bridge2.0-0 \
    libxcomposite1 \
    libxdamage1

# 复制项目
WORKDIR /app
COPY . .

# 安装依赖
RUN pip install -r requirements.txt
RUN npm install
RUN npx playwright install chromium

# 启动
CMD ["python", "src/ui/app.py"]
```

### 9.3 服务器部署

**nginx 反向代理**:
```nginx
server {
    listen 80;
    server_name auto-test.example.com;

    location / {
        proxy_pass http://127.0.0.1:7860;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
}
```

**systemd 服务**:
```ini
[Unit]
Description=Auto Test Service
After=network.target

[Service]
Type=simple
User=webapp
WorkingDirectory=/opt/midscene-auto-test
ExecStart=/opt/midscene-auto-test/venv/bin/python src/ui/app.py
Restart=on-failure

[Install]
WantedBy=multi-user.target
```

---

## 10. 监控和日志

### 10.1 日志系统

**日志配置**:
```python
# src/utils/logger.py
import logging
import os
from datetime import datetime

def setup_logger(name: str) -> logging.Logger:
    """配置日志"""

    logger = logging.getLogger(name)
    logger.setLevel(logging.INFO)

    # 文件处理器
    log_dir = os.getenv('LOG_DIR', './logs')
    os.makedirs(log_dir, exist_ok=True)

    log_file = os.path.join(
        log_dir,
        f'app_{datetime.now().strftime("%Y%m%d")}.log'
    )

    file_handler = logging.FileHandler(log_file, encoding='utf-8')
    file_handler.setLevel(logging.INFO)

    # 控制台处理器
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.INFO)

    # 格式化
    formatter = logging.Formatter(
        '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
    )
    file_handler.setFormatter(formatter)
    console_handler.setFormatter(formatter)

    logger.addHandler(file_handler)
    logger.addHandler(console_handler)

    return logger
```

**使用示例**:
```python
from utils.logger import setup_logger

logger = setup_logger('TestCaseManager')

logger.info('开始执行测试用例')
logger.warning('检测到警告')
logger.error('执行失败', exc_info=True)
```

### 10.2 性能监控

**执行时间统计**:
```python
import time
from functools import wraps

def timing_decorator(func):
    """执行时间装饰器"""
    @wraps(func)
    def wrapper(*args, **kwargs):
        start = time.time()
        result = func(*args, **kwargs)
        duration = time.time() - start
        logger.info(f'{func.__name__} 执行时间: {duration:.2f}s')
        return result
    return wrapper

@timing_decorator
def execute_yaml(yaml_content):
    # 执行逻辑
    pass
```

### 10.3 错误追踪

**错误统计**:
```python
class ErrorTracker:
    """错误追踪器"""

    def __init__(self):
        self.errors = []

    def log_error(self, category: str, message: str):
        """记录错误"""
        self.errors.append({
            'category': category,
            'message': message,
            'timestamp': datetime.now().isoformat()
        })

    def get_summary(self) -> dict:
        """获取错误摘要"""
        from collections import Counter
        categories = [e['category'] for e in self.errors]
        return {
            'total': len(self.errors),
            'by_category': dict(Counter(categories))
        }
```

---

## 总结

### 技术亮点

1. ✅ **跨语言集成**: Python + Node.js 无缝协作
2. ✅ **模块化设计**: 清晰的职责划分,易于维护
3. ✅ **复用成熟框架**: 最大化利用 Midscene.js 能力
4. ✅ **安全可靠**: 完善的错误处理和安全措施
5. ✅ **可扩展性强**: 为后续迭代预留接口

### 技术风险

| 风险 | 等级 | 缓解措施 |
|------|------|---------|
| Python-Node.js 通信不稳定 | 中 | 完善错误处理、日志记录 |
| LLM 生成质量不稳定 | 高 | 迭代优化 Prompt、人工审核 |
| 浏览器兼容性问题 | 低 | 提供 Puppeteer 备选方案 |

### 后续优化方向

**短期**:
- 添加更详细的日志
- 优化错误提示
- 提升 UI 响应速度

**中期**:
- 实现 HTTP API 替代 subprocess
- 添加 Redis 缓存
- 支持分布式执行

**长期**:
- 微服务架构改造
- Kubernetes 部署
- 监控告警系统

---

**文档结束**

© 2025 孙顺达 版权所有
